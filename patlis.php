      ...
      <div class="column1 right1">
          ...
          <div class="tab">
              <form style="padding-top:20px"action="<?php $_SERVER['PHP_SELF']; ?>" method="post">
               <input required class="form-control" type="text" name="pid" placeholder="Enter Patient ID or Name">
               <input class="btn btn-primary" style="background-color:black;color:white;margin-top: 3px;" id="register" type="submit" name="pdet" value="Show Patient Details">
              </form>
           <br>
           <?php
             if(isset($_POST['pdet']))
             {
               ...
               $sql=" SELECT * FROM patient where pid='$data' OR pname LIKE '%".$data."%' ";
                     ...
                     <?php
                      ...
                      ?>
                             <tr>
                                   ...
                                   <td>   <?php echo $row['pmail']; ?>      </td>
                                   <td>   <?php echo $row['paddress']; ?>   </td>
                                   <td style="text-align:center;">
                                          <a href="patedi.php?id=<?php echo $row['pid']; ?>">Edit</a>
                                   </td>
                             </tr>
                    <?php
                           }
               ...
               { ?>
                 <h1><center>No Patient present with given ID or Name</center></h1>
                 <?php
               }
             }
             else
               ...
               <?php
                 ...
                 else
                 {
                   $pag=1;
                       ...
                       $num_per_page=10;
                       $start_from=($pag-1)*10;
                       $sql=" SELECT * FROM patient ORDER BY pid DESC LIMIT $start_from,$num_per_page";
                       $result=mysqli_query($conn,$sql) or die("Query Unsuccessfull.");
                       if(mysqli_num_rows($result) > 0)
                       {
                         ?>
